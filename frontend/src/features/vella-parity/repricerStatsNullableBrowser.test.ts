...
it('renders unknown and zero baskets honestly in both actual statistics pages', async () => {
  ...
  try {
    ...
    expect(await page.locator('#repricerStatsBody tr').first().locator('td').nth(5).innerText()).toBe('—')
    expect(await page.locator('#repricerStatsBody tr').first().locator('td').nth(6).innerText()).toBe('—')
    ...
    await expectRow(page, 'ZERO-STATS-SKU', 5, 6, '0', '0%')
    await expectRow(page, 'NULL-CR-STATS-SKU', 5, 6, '5', '—')
    await expectRow(page, 'POSITIVE-STATS-SKU', 5, 6, '7', '42,9%')
