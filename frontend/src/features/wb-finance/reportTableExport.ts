...
export const PNL_TABLE_EXPORT_HEADERS = ['Товар', 'Категория', 'Выручка', 'Себестоимость', 'Комиссия', 'Логистика', 'Хранение', 'Реклама', 'Налог', 'Прибыль после лояльности', 'Финальная маржа', 'Статус', 'Комментарий']
...
type PnlRow = {
  ...
  taxKopecks?: number | null; profitAfterLoyaltyKopecks?: number | null; marginPct?: number | null
...
export function buildPnlTableRows(rows: PnlRow[]): Cell[][] {
  return rows.map(row => {
    ...
    return [
      ...
      row.category ?? null,
      ...[row.revenueKopecks, row.cogsKopecks, row.commissionKopecks, row.logisticsKopecks, row.storageKopecks, row.adSpendKopecks, row.taxKopecks, row.profitAfterLoyaltyKopecks].map(rubles),
      row.marginPct ?? null, canonicalPnlRowStatus(row), row.comment ?? '',
    ]
  })
}

type AbcRawRow = { sku?: string | null; nmId?: number | string | null; cogsPerUnitKopecks?: number | null; adSpendKopecks?: number | null; profitAfterLoyaltyKopecks?: number | null; blockerIds?: string[] | null }
...
export function buildAbcTableRows(rawRows: AbcRawRow[], displayRows: AbcDisplayRow[], snapshot: { count: number; rows: Array<{ row: AbcDisplayRow; originalIndex: number }> }, columns: string[]): Cell[][] {
  ...
  return snapshot.rows.map(({ row, originalIndex }) => {
    ...
    const cells: Record<string, Cell> = {
      ...
      ads: raw.adSpendKopecks == null ? null : String(row.ads ?? ''), net: rubles(raw.profitAfterLoyaltyKopecks),
