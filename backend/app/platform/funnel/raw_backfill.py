...
from app.platform.funnel.raw import FunnelNormalizationError
...
from app.wb_api.raw_funnel import (
    RawFunnelFetchError,
    fetch_raw_funnel,
)


class RawFunnelBackfillError(ValueError):
    ...
    *,
    ...
    if credential_ref is None:
        ...
        if marketplace_account_id is not None and account_id != marketplace_account_id:
            raise RawFunnelBackfillError("wb_account_changed")
    else:
        try:
            account_id = resolve_bound_wb_credential(
                session,
                organization_id,
            ...
            raise RawFunnelBackfillError("wb_credential_binding_invalid") from None
    external_ids = session.scalars(
        select(MarketplaceProductRow.external_product_id).where(
            MarketplaceProductRow.organization_id == organization_id,
            MarketplaceProductRow.marketplace_account_id == account_id,
        )
    ).all()
    nm_ids = sorted(
        {
            ...
            if (value := str(raw).strip()).isdigit() and int(value) > 0
        }
    )
    if not nm_ids:
        raise RawFunnelBackfillError("wb_products_missing")
    return account_id, nm_ids
...
def _backfill(
    ...
    try:
        fetched = fetch_raw_funnel(period, nm_ids, wb_token=wb_token)
    except RawFunnelFetchError:
        raise RawFunnelBackfillError("wb_fetch_failed") from None
    ...
    try:
        current_account_id, _current_nm_ids = _scope(
            session,
            organization_id,
            account_id,
            credential_ref,
        ...
        snapshot = FunnelService(session, organization_id).ingest_raw_payload(
            current_account_id,
            period,
            fetched.bundle,
...
def backfill_raw_funnel(
    ...
    *,
    ...
    if session is not None and session.in_transaction():
        raise RawFunnelBackfillError("session_transaction_active")
    if organization_id < 1:
        raise RawFunnelBackfillError("invalid_organization")
    token = (wb_token or "").strip()
    if not token:
        token = (get_organization_wb_token_secret(organization_id) or "").strip()
    if not token:
        raise RawFunnelBackfillError("wb_token_missing")
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Backfill canonical WB Sales Funnel daily evidence"
    )
