...
**Files:**
- Create: `app/report_data_mart.py`
- Create: `tests/test_report_data_mart.py`

**Interfaces:**
- Produces: `SourceCoverage` dataclass.
- Produces: `ReportSourceBundle` dataclass.
- Produces: `load_report_source_bundle(organization_id: int, date_from: date, date_to: date, required_sources: Iterable[str], get_cache: Callable[..., dict[str, Any] | None] = get_source_cache) -> ReportSourceBundle`.
- Produces: `source_cache_covers_range(cache: Mapping[str, Any], date_from: date, date_to: date) -> bool`.
- Produces: `source_cache_matches_range(cache: Mapping[str, Any], date_from: date, date_to: date) -> bool`.

- [ ] **Step 1: Write failing data mart tests**

Add `tests/test_report_data_mart.py`:
...
Run: `pytest -q tests/test_report_data_mart.py`

Expected: FAIL with `ModuleNotFoundError: No module named 'app.report_data_mart'`.

- [ ] **Step 3: Implement the data mart reader**
...
```bash
git add app/report_data_mart.py tests/test_report_data_mart.py
git commit -m "feat: add report data mart coverage reader"
```
