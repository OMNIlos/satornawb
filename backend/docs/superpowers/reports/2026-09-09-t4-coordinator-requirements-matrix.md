...
| 5. Notifications | Actual0074 participant + atomic send/local-draft producers and own-recipient live-session read/mark/capability root implemented UNVERIFIED; exact account allowlist empty by default | Earlier287purePASS covers codecs only; new service/RLS/concurrent merge acceptance PENDING | T4 discovery/list and dormant HTTP/frontend integration; T1 preferences CAS extension; external delivery policy separate | No Telegram/provider actions; old0071 writer unchanged/no replay backfill |
