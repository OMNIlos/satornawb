...
| 4. Send/recovery | e4818e3 recovery/pure codecs plus actual0074 private bindings/audit-first SQL participant, implemented UNVERIFIED; complete parent chain consumed throughc611c4c | Earlier287purePASS covers codecs only, NOT new repository/0074; no new gate | T1 multi-member/executor/closing authority; T4 guarded intent→claim→committed marker→fake provider→receipt/recovery chain, uncertain commit/restart/enqueue; actual0074 DDL no longer missing | Live sends off; production policy separate from local fake-provider acceptance |
| 5. Notifications | Safe projection/preferences/codecs plus actual0074 event/dedupe and explicit-visible receipt SQL participant, implemented UNVERIFIED | Earlier287purePASS covers codecs only; new repository/RLS/concurrent merge acceptance PENDING | T4 guarded source+event atomic producer and authenticated recipient root/read/capability integration; T1 preferences CAS extension; external delivery policy separate | No Telegram/provider actions; no old0071 hooks/backfill |
