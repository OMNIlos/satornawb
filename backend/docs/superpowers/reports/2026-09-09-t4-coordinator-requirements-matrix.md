...
| 4. Send/recovery | Actual0074 participant plus guarded T1 roots consumed297fcfd→b6b6343; T4 source/policy/approval callbacks, closing and queued-only one-POST driver implemented UNVERIFIED | Earlier287purePASS covers codecs only, NOT new repositories/authority/services/driver; no new gate | T1 sealed reconciliation intent and original-sender cancel follow-ups; T4 fresh GET/cancel composition, concrete synthetic transport acceptance, uncertain commit/restart/enqueue; broad authority/0074 DDL no longer missing | Live sends off; no provider bootstrap or final acceptance |
| 5. Notifications | Actual0074 event/receipt participant plus atomic send-blocked/ambiguous producer and explicit NEW local-draft producer implemented UNVERIFIED | Earlier287purePASS covers codecs only; new service/RLS/concurrent merge acceptance PENDING | T4 authenticated recipient root/read/capability integration; T1 preferences CAS extension; external delivery policy separate | No Telegram/provider actions; old0071 writer unchanged/no replay backfill |
