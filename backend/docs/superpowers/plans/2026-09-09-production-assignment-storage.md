...
Before the CHECK's casts, a BEFORE INSERT validator must check object key sets, JSON types and integer lexical/width/range bounds; require canonical integer lexical forms (not bool/string/fraction/exponent/NULL), exact request expected=result_version-1, exact work item/key. Do not depend on SQL CHECK evaluation order for safe parsing. No silent numeric coercion. Trigger helpers cannot be runtime callable mutation APIs.
